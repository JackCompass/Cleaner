...
def RemoveSimilarImage(folder_path, hash = utility.hashlib.sha1):
	...
	for dirpath, dirname, files in os.walk(folder_path):
		...
		for file in files:
			...
			file_size[os.path.getsize(clear_path)].append(clear_path)

	for _, files in file_size.items():
		...
		if len(files) < 2:
			continue
		for file in files:
			meta_hash = utility.generate_hash(file, True)
			hash_meta[meta_hash].append(file)

	for meta_hash, files in hash_meta.items():
		...
		for file in files:
			try:
				full_hash = utility.generate_hash(file, False)
				...
				# File access might have changed
				continue
...
if __name__ == "__main__":
	...
	if sys.argv[1:]:
		...
		RemoveSimilarImage(path)
	else:
		print("Folder must be present in the current working directory. Execute with folder name")
